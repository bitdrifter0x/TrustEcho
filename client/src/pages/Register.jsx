...
export default function Register() {
  ...
  return (
    <div className="min-h-screen bg-[#060A14] flex flex-col items-center justify-center text-slate-100 font-sans overflow-x-hidden py-12 px-4 relative">
      
      {/* Background Ambient Glow */}
      <div 
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full pointer-events-none -z-10"
        style={{ background: 'radial-gradient(circle, rgba(139,92,246,0.1) 0%, transparent 65%)', width: '600px', height: '600px' }} 
      />
      ...
      <div className="relative overflow-hidden bg-slate-900/40 border border-slate-800 p-8 rounded-2xl w-full max-w-md shadow-[0_24px_64px_rgba(0,0,0,0.4)] backdrop-blur-xl">
        <div style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(139,92,246,0.06) 0%, transparent 60%)' }} className="absolute inset-0 pointer-events-none" />
        
        <div className="relative z-10">
          <h2 className="text-3xl font-black text-center tracking-tight mb-2">
            Create your account
          </h2>
          <p className="text-slate-400 text-xs font-medium tracking-wide text-center mb-6">START COLLECTING AND DISPLAYING TESTIMONIALS TODAY</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">
                Company Name <span className="text-slate-600 font-normal lowercase">(optional)</span>
              </label>
              <input 
                type="text" 
                className="w-full bg-[#060A14] border border-slate-800 rounded-xl p-3 text-sm text-white focus:outline-none focus:border-indigo-500/60 transition-colors placeholder-slate-700" 
          ...
          <div className="border-t border-slate-800/60 pt-5 mt-6 text-center">
            <p className="text-xs text-slate-400 font-medium">
              ...
              <Link to="/login" className="text-violet-400 hover:text-indigo-400 font-bold transition-colors ml-1">
                Log in here
              </Link>
            </p>
          </div>
